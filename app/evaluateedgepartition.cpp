...
███████ ██████   ██████  ███████
    ...
    if(replicationFactor == 0) {
        ...
        auto cfg = edgePartition->partition_configuration();
        auto updatedEdgePartition = PartitionInfo::CreateEdgePartition(
                ...
                PartitionInfo::CreatePartitionConfiguration(builder,
                                                           cfg->k(),
                                                           cfg->seed(),
                                                           cfg->batch_size(),
                                                           cfg->model_mode(),
                                                           ...
                                                           cfg->max_buffer_size(),
                                                           cfg->bb_ratio(),
                                                           cfg->buffer_score_type(),
                                                           cfg->d_max(),
                                                           cfg->haa_beta(),
                                                           cfg->haa_theta(),
                                                           cfg->cbs_theta(),
                                                           cfg->buffer_neighbor_weight(),
                                                           cfg->batch_frontier_weight(),
                                                           cfg->num_streams_passes(),
                                                           cfg->restream_number(),
                                                           cfg->restream_include_high_degree_nodes(),
                                                           cfg->restream_vcycle(),
                                                           cfg->ghost_neighbors_enabled(),
                                                           cfg->ghost_weight(),
                                                           cfg->sep_batch_marker(),
                                                           cfg->batch_extraction_strategy(),
                                                           cfg->max_active_batches(),
                                                           cfg->max_input_q_size(),
                                                           cfg->alt_thread_queue(),
                                                           cfg->collect_locality_metrics()),
